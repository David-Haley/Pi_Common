...
--  20260621 : AD_Sample_Pin pulse polarity inverted. Because the ADC needs a
--  conversion pulse less than 500 ns this has to be generated by an external
--  monostable, the minimum pulse width using the curent GPIO driver being of
--  the order of units of micro seconds. The previous logic was correct if the
--  conversion pin was being driven directly; however in the
--  rpi-hot-water-controller the B input of a 74LV221 is used resulting in a
--  logical inversion (positive edge triggered). This means thet conversion
--  occurs afrer control is returned to this device driver. There os no
--  feedback that the conversion is complete so there is
--  reliance on delay before the SPI read. Making the AD_Sample_Pin pulse
--  positive going (normally low) the conversion will occur before the GPIO
--  driver call returns, guaranteeing some time for the concersion to complete
--  before the SPI read. May also reduce jitter in conversion slightly.
...
package body AD7091R2 is
   ...
   begin -- Initialize
      ...
      Strobe_Low (SPI_State.Reset_Pin);
      -- reset A/D must be more than 10ns
      Bind_Pin (SPI_State.Sample_Pin, Out_Pin);
      Write_Pin (Pin_Low, SPI_State.Sample_Pin); -- /CONVST is active low
      -- Monostable triggered on rising edge
